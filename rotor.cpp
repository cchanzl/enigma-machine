...
void load_rotor_pos(const char* filename, int pos_array[], int number_of_rotors){
  ...
  while(in >> input){
    ...
    if ( setting < 0 || setting > 25 ) {
      cout << "Error: Position config contains a number not between 0 and 25";
      exit(INVALID_INDEX);
    }
  ...
  if ( count < number_of_rotors ){   // comparison is ok as the last loop +1 before exiting 
    cout << "No starting position for rotor " << count  << " in rotor position file: rotor.pos";
...
void Rotor::load_rotor_setting(const char* filename, const int pos_array[], const int rotor_pos){
  ...
  while(in >> input){
    ...
    // check if a is a valid index
    if ( setting < 0 || setting > 25 ) {
      cout << "Error: Rotor config contains a number not between 0 and 25";
      exit(INVALID_INDEX);
    }
	...
	if ( rotor_mapping[count] == rotor_mapping[i] ) {
	  ...
	  cout << "Invalid mapping of input " << count << " to output " << rotor_mapping[count] << " (output " << rotor_mapping[count] << " is already mapped to from input " << i  << ")"; 
